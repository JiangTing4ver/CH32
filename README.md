...
这是一个基于 CH32F03C8T6 微控制器的多功能开发项目，使用了 WCH（南京沁恒）提供的标准外设库。项目采用模块化设计，已实现多个实用功能模块，包括基础 LED 控制、传感器数据采集、串口通信以及 WS2812 灯带控制。项目结构清晰，易于扩展，适合嵌入式开发学习和二次开发。（持续更新中）
...
### OLED 显示模块
- **功能**: 实现 OLED 屏幕的初始化和字符显示
- **文件**: `MyLib/OLED_Data.c` 和 `MyLib/OLED_Data.h`
- **使用**: 在 `main.c` 中定义 `#define flag OLED` 启用
- 支持中文|英文|图片 字符显示
